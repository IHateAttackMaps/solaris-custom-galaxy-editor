...
class IrregularMapGenerator implements MapGenerator {
    ...
    _generateHomeLocations(pivotDistance: number, playerCount: number, rand: SeededRandomGen, simplexNoiseGenerator: NoiseFunction2D, noiseSpread: number) {
        ...
        while (homeLocations.length < playerCount) {
            ...
            while (!positionIsValid) {
                ...
                for (let homeLocation of homeLocations) {
                    if (helper.getDistanceBetweenLocations(position, homeLocation) < pivotDistance) {
