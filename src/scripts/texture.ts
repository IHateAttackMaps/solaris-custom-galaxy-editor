...
const TEXTURE_URLS = {
    ...
    SELECTION_ROTATE: new URL('../assets/selection/rotate-small-right.svg', import.meta.url).href
};
...
class TextureService {
    ...
    SPECIALIST_TEXTURES = {} as any;
    PLAYER_SYMBOLS = {} as any;
    STAR_SYMBOLS = {} as any;
    ...
    SELECTION_ROTATE_TEXTURE: Texture | undefined;

    async loadAssets() {
        await Promise.all(Object.keys(TEXTURE_URLS).map(spec => {
            const val = (TEXTURE_URLS as any)[spec];
    ...
    _loadSelectionTextures() {
        this.SELECTION_ROTATE_TEXTURE = Texture.from(TEXTURE_URLS.SELECTION_ROTATE);
    }

    getRandomStarNebulaTexture(seed: string) {
    ...
    getRandomStarAsteroidFieldTexture(seed: string) {
